...
std::string vmessLinkConstruct(const std::string &remarks,
...
std::string proxyToClash(std::vector<Proxy> &nodes,
  ...
  try {
    ...
    writeLog(0, std::string("Clash base loader failed with error: ") + e.what(),
             LOG_LEVEL_ERROR);
    return "";
  }
...
std::string proxyToSurge(std::vector<Proxy> &nodes,
  ...
  if (ini.parse(base_conf) != 0 && !ext.nodelist) {
    writeLog(0, "Surge base loader failed with error: " + ini.get_last_error(),
             LOG_LEVEL_ERROR);
    return "";
  }
...
std::string proxyToSingle(std::vector<Proxy> &nodes, int types,
  ...
  for (Proxy &x : nodes) {
    ...
    std::string &hostname = x.Hostname, &sni = x.ServerName,
                ...
                &faketype = x.FakeType, &ports = x.Ports;
    bool &tlssecure = x.TLSSecure;
    ...
    case ProxyType::VMess:
      if (!vmess)
        continue;
      proxyStr =
          "vmess://" + base64Encode(vmessLinkConstruct(
                           remark, hostname, port, faketype, id, aid,
                           transproto, path, host, tlssecure ? "tls" : ""));
      break;
    case ProxyType::Hysteria2:
      if (!hysteria2)
        continue;
    ...
    case ProxyType::Trojan:
      if (!trojan)
        continue;
      proxyStr = "trojan://" + password + "@" + hostname + ":" + port +
                 "?allowInsecure=" + (x.AllowInsecure.get() ? "1" : "0");
      if (!sni.empty()) {
        proxyStr += "&sni=" + sni;
      } else if (!host.empty()) {
        proxyStr += "&sni=" + host;
...
std::string proxyToSSSub(std::string base_conf, std::vector<Proxy> &nodes,
  ...
  if (!result)
    writeLog(0,
             std::string("SIP008 base loader failed with error: ") +
...
std::string proxyToQuan(std::vector<Proxy> &nodes, const std::string &base_conf,
  ...
  if (!ext.nodelist && ini.parse(base_conf) != 0) {
    writeLog(
        0, "Quantumult base loader failed with error: " + ini.get_last_error(),
        LOG_LEVEL_ERROR);
    return "";
  }
...
std::string proxyToQuanX(std::vector<Proxy> &nodes,
  ...
  if (!ext.nodelist && ini.parse(base_conf) != 0) {
    writeLog(
        0, "QuantumultX base loader failed with error: " + ini.get_last_error(),
        LOG_LEVEL_ERROR);
    return "";
  }
...
std::string proxyToMellow(std::vector<Proxy> &nodes,
  ...
  if (ini.parse(base_conf) != 0) {
    writeLog(0, "Mellow base loader failed with error: " + ini.get_last_error(),
             LOG_LEVEL_ERROR);
    return "";
  }
...
std::string proxyToLoon(std::vector<Proxy> &nodes, const std::string &base_conf,
  ...
  if (ini.parse(base_conf) != INIREADER_EXCEPTION_NONE && !ext.nodelist) {
    writeLog(0, "Loon base loader failed with error: " + ini.get_last_error(),
             LOG_LEVEL_ERROR);
    return "";
  }
...
buildSingBoxTransport(const Proxy &proxy,
  ...
  case "ws"_hash: {
    transport.AddMember("type",
                        rapidjson::StringRef(proxy.TransferProtocol.c_str()),
                        allocator);
    if (proxy.Path.empty())
    ...
    case ProxyType::VLESS: {
      ...
      case "ws"_hash:
        if (x.Path.empty())
          ...
          vlesstransport.AddMember("path", rapidjson::StringRef(x.Path.c_str()),
                                   allocator);
        if (!x.Host.empty())
          vlessheaders.AddMember("Host", rapidjson::StringRef(x.Host.c_str()),
                                 allocator);
        if (!x.Edge.empty())
          vlessheaders.AddMember("Edge", rapidjson::StringRef(x.Edge.c_str()),
                                 allocator);
        ...
        break;
      case "http"_hash:
        vlesstransport.AddMember("type", rapidjson::StringRef("http"),
                                 allocator);
        vlesstransport.AddMember("host", rapidjson::StringRef(x.Host.c_str()),
                                 allocator);
        vlesstransport.AddMember("method", rapidjson::StringRef("GET"),
                                 allocator);
        vlesstransport.AddMember("path", rapidjson::StringRef(x.Path.c_str()),
                                 allocator);
        addHeaders(vlesstransport, x, allocator);
        ...
        break;
      case "h2"_hash:
        vlesstransport.AddMember("type", rapidjson::StringRef("httpupgrade"),
                                 allocator);
        vlesstransport.AddMember("host", rapidjson::StringRef(x.Host.c_str()),
                                 allocator);
        vlesstransport.AddMember("path", rapidjson::StringRef(x.Path.c_str()),
        ...
        break;
      case "grpc"_hash:
        vlesstransport.AddMember("type", rapidjson::StringRef("grpc"),
                                 allocator);
        vlesstransport.AddMember(
            "service_name", rapidjson::StringRef(x.GRPCServiceName.c_str()),
            allocator);
        proxy.AddMember("transport", vlesstransport, allocator);
        break;
      default:
    ...
    if (x.TLSSecure) {
      ...
      if (x.Type == ProxyType::VLESS) {
        ...
        if (!x.PublicKey.empty() || !x.ShortId.empty()) {
          rapidjson::Value utls(rapidjson::kObjectType);
          utls.AddMember("enabled", true, allocator);
          utls.AddMember("fingerprint", rapidjson::StringRef("chrome"),
                         allocator);
          tls.AddMember("utls", utls, allocator);
          ...
          //                    auto shortIds =
          //                    stringArrayToJsonArray(x.ShortId, ",",
          //                    allocator);
          if (!x.ShortId.empty()) {
            reality.AddMember(
                "short_id", rapidjson::StringRef(x.ShortId.c_str()), allocator);
          } else {
...
std::string proxyToSingBox(std::vector<Proxy> &nodes,
  ...
  if (!ext.nodelist) {
    ...
    if (json.HasParseError()) {
      writeLog(
          0,
          "sing-box base loader failed with error: " +
